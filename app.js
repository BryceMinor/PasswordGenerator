...
// Function that returns a random symbol (AKA Random string value of a number)
...
console.log(getRandomSymbol()); // Random symbol
...
// Function that accepts true or false as values as well as a number as arguments
...
function generatePassword(lower, upper, number, symbol, length){
    ...
    // Creating an array of arrays. The first itrm in each nested array holds the value of a string that will be used to access a function in the randomFunctions object. Also, the second item in each nested array is one of the values passed into this generatePassword function.
    let typesArr = [
        [`lower`, lower],
        [`upper`, upper],
        [`number`, number],
    ...
    // The filter method creates a new array with all the elements that pass the test implemented by the provided function (AKA all the items that cause the function to return a boolean value of true when the function is run using the item as an argument for the item parameter in this example)
...
// NOTE: Using the starting values for when ths page first loads
...
generateEl.addEventListener(`click`, () => {
    ...
    // The generatePassword function takes the true/false values determined by the checkboxes as well as the number from the number input as arguments and returns strings (AKA The Password) which is set as the innerText value for the "result" (AKA Span) element
...
clipboardEl.addEventListener(`click`, () => {
    ...
    // Adding the text area to the webpage/document
    ...
    // NOTE: Some execCommand commands/arguments are not supported by all browsers. However, copy (and cut) is an execCommand command that all browsers support.
